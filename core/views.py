...
@api_view(['POST'])
@permission_classes([AllowAny])
def initialize_membres(request):
    """
    Endpoint pour creer les membres de la mutuelle via le script ajouter_profs.py
    
    POST /api/core/scripts/initialize-membres/
    ...
    try:
        ...
        if result.returncode == 0:
            return Response({
                "status": "success",
                "message": "Initialisation des membres exécutée avec succès",
                "output": result.stdout,
            }, status=status.HTTP_200_OK)
        else:
            return Response({
