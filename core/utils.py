...
def calcul_tresor():
    """
    Calcule le trésor total de la caisse à partir des transactions d'épargne.
    
    Formule: DEPOT + RETOUR_REMBOURSEMENT - |RETRAIT_PRET|
    
    Returns:
        Decimal: Montant total du trésor en FCFA
    """
    from transactions.models import EpargneTransaction
    
    # Dépôts (positifs)
    depots = EpargneTransaction.objects.filter(
        type_transaction='DEPOT'
    ).aggregate(
        total=Sum('montant')
    )['total'] or Decimal('0')
    
    # Retours de remboursement (positifs)
    retours = EpargneTransaction.objects.filter(
        type_transaction='RETOUR_REMBOURSEMENT'
    ).aggregate(
        total=Sum('montant')
    )['total'] or Decimal('0')
    
    # Retraits pour prêt (négatifs dans la DB)
    retraits = EpargneTransaction.objects.filter(
    ...
    # Calcul du trésor
    tresor = depots + retours - abs(retraits)
...
def calculer_donnees_administrateur():
    ...
    return {
        ...
        'tresor': {
            'tresor_total': tresor_montant,
            'cumul_total_epargnes': tresor_montant
        },
        'emprunts_en_cours': {
        ...
        'situation_globale': {
            'liquidites_totales': fonds_social['montant_total'] + tresor_montant,
